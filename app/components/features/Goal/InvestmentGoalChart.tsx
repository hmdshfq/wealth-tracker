...
const CustomTooltip: React.FC<CustomTooltipProps> = ({
  ...
  const renderEntries = (entries: TooltipPayloadItem[]) => {
    const uniqueEntries = entries.reduce<TooltipPayloadItem[]>((acc, entry) => {
      ...
      // Recharts tooltip may include both Area and Line entries for the same dataKey.
      // Prefer the stroked (Line) entry so label color/styling matches the visible boundary line.
    ...
    return uniqueEntries
      ...
      .map(({ entry, index }) => {
        ...
        const monteCarloFallbackColor =
          (normalizedKey.includes('p90') || normalizedKey.includes('90%')) ? monteCarloColorLookup.p90 :
          (normalizedKey.includes('p50') || normalizedKey.includes('50%') || normalizedKey.includes('median')) ? monteCarloColorLookup.p50 :
          (normalizedKey.includes('p10') || normalizedKey.includes('10%')) ? monteCarloColorLookup.p10 :
          undefined;
