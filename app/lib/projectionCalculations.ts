import { Goal, ProjectionDataPoint, Transaction, MonteCarloSimulationResult, InvestmentScenario, ScenarioAnalysisResult, RiskMetrics, RiskAnalysisResult, SeasonalPattern, YoYComparison, TimeBasedAnalysisResult } from './types';
...
// ============================================
// TIME-BASED ANALYSIS CALCULATIONS
// ============================================
...
function calculatePatternStrength(returns: number[]): number {
  ...
  const consistency = returns.filter(r => (r > 0 && meanReturn > 0) || (r < 0 && meanReturn < 0)).length / returns.length;
